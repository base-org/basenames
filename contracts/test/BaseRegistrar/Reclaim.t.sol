...
contract Reclaim is BaseRegistrarBase {
    ...
    // function test_reclaimsOwnership_whenCalledByOwner_duringGracePeriod() public {
    //     _registrationSetup();
    //     uint256 expires = _registerName(label, user, duration);
    ...
    // function test_reclaimsOwnership_whenCalledByOperator_duringGracePeriod(address operator) public {
    //     vm.assume(operator != address(0) && operator != user);
    //     _registrationSetup();
